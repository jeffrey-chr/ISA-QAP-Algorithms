...
#define ASSIGN_ENUM_PARAMETER(PARAM, TYPE)                                     \
    do {                                                                       \
        ...
        {                                                                      \
            ...
            if (options.arg_##PARAM) {                                         \
                printf ("with argument \"%s\"\n", strings_##TYPE[opt]);        \
            }                                                                  \
        } else {                                                               \
            fprintf(stdout,"\tNote: --%s is set to default \"%s\"\n",          \
                    optstr__##PARAM, strings_##TYPE[PARAM]);                   \
        }                                                                      \
    } while(0)                                                                 \
    ...
    do {                                                                       \
        if ( options.opt_##PARAM ) {                                           \
            ...
            fprintf(stdout, (optchar__##PARAM[0] != '\0')                      \
                    ? "  -%s  --%s " : "  %2s  --%s ",                         \
                    optchar__##PARAM, optstr__##PARAM);                        \
            if (options.arg_##PARAM != NULL)                                   \
                printf ("with argument %g\n", PARAM);                          \
            check_out_of_range (PARAM, MIN_RANGE, MAX_RANGE, optstr__##PARAM); \
        } else {                                                               \
            fprintf(stdout,"\tNote: --%s is set to default %g\n",              \
                    optstr__##PARAM, PARAM);                                   \
        }                                                                      \
    } while(0)
...
#define ASSIGN_INTEGER_PARAMETER(PARAM, MIN_RANGE, MAX_RANGE)                  \
    do {                                                                       \
        if ( options.opt_##PARAM ) {                                           \
            ...
            fprintf(stdout,"\tNote: --%s is set to default %ld\n",             \
                    optstr__##PARAM, PARAM);                                   \
        }                                                                      \
    } while(0)
    ...
    do {                                                                       \
        if (options.opt_##PARAM) {                                             \
            ...
            fprintf(stdout, (optchar__##PARAM[0] != '\0')                      \
                    ? "  -%s  --%s " : "  %2s  --%s ",                         \
                    optchar__##PARAM, optstr__##PARAM);                        \
            fprintf(stdout, "is set to 1\n");                                  \
        } else {                                                               \
            fprintf(stdout,"\tNote: --%s is set to default %d\n",              \
                    optstr__##PARAM, (int) PARAM);                             \
        }                                                                      \
    } while(0)
